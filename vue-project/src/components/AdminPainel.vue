    ...
    <main class="admin-main">
      <section v-if="abaAtiva === 'pendentes'" class="admin-section">
        ...
        <div class="grid-pendentes">
          <div v-for="item in pendentes" :key="item.id + item.tipo_item" class="pendente-card">
            <div class="card-header">
              <span class="tag-nicho">{{ item.nicho }}</span>
              <span class="tag-tipo">{{ item.tipo_item === 'projeto' ? '🏢 Projeto' : '💡 Ideia' }}</span>
            </div>
            <h3 class="card-empresa">{{ item.empresa || item.titulo }}</h3>
            <p class="card-desc">{{ item.descricao }}</p>
            <div class="card-actions">
              <button @click="aprovar(item)" class="btn-aprovar">✅ Aprovar</button>
              <button @click="rejeitar(item)" class="btn-rejeitar">❌ Rejeitar</button>
            </div>
          </div>
        </div>
      </section>
...
const aprovar = async (item) => {
  if (!confirm(`Aprovar "${item.empresa || item.titulo}"?`)) return;
  try {
    await axios.put(`https://conexo-api.onrender.com/admin/aprovar/${item.id}`);
...
const rejeitar = async (item) => {
  ...
  try {
    await axios.delete(`https://conexo-api.onrender.com/${rota}/${item.id}`);
    await carregarPendentes();
...
.tag-tipo { font-size: 0.75rem; color: #8b949e; }
